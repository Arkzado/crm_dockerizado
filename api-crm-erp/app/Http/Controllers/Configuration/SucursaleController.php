...
class SucursaleController extends Controller
{
    ...
    {
        $is_exist_sucural = Sucursale::where("name", $request->name)->first();

        if ($is_exist_sucural) {
            return response()->json([
                "message" => 403,
                "message_text" => "El nombre de la sucursal ya está registrado",
            ]);
        }
    ...
    {
        $is_exist_sucural = Sucursale::where("name", $request->name)->where("id", "<>", $id)->first();
