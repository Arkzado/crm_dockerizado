...
export class RolesService {
  ...
  registerRole(data: any) {
    ...
    return this.http.post(URL, data, { headers }).pipe(finalize(() => this.isLoadingSubject.next(false)));
  }
  ...
  listRoles(page = 1, search: string = '') {
    ...
    return this.http.get(URL, { headers }).pipe(finalize(() => this.isLoadingSubject.next(false)));
  }
  ...
  deleteRole(ID_ROLE: string) {
    ...
    let headers = new HttpHeaders({ 'Authorization': 'Bearer ' + this.authservice.token });
    let URL = URL_SERVICIOS + "/roles/" + ID_ROLE;
    return this.http.delete(URL, { headers }).pipe(finalize(() => this.isLoadingSubject.next(false)));
  }
}
