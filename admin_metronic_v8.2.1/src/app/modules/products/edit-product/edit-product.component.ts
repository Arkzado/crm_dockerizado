...
export class EditProductComponent {
  ...
  ngOnInit(): void {
    ...
    this.productService.showProduct(this.PRODUCT_ID).subscribe((resp: any) => {
      ...
      this.tiempo_de_abastecimiento =
        this.PRODUCT_SELECTED.tiempo_de_abastecimiento;
  ...
  addWarehouse() {
    if (
      ...
      this.toast.error(
        'VALIDACIÓN',
        'Necesitas seleccionar un almacen y una unidad, aparte de colocar una cantidad'
      );
      return;
    }

    let UNIT_SELECTED = this.UNITS.find(
      (unit: any) => unit.id == this.unit_warehouse
    );
    let WAREHOUSE_SELECTED = this.WAREHOUSES.find(
      (wareh: any) => wareh.id == this.almacen_warehouse
    );

    let INDEX_WAREHOUSE = this.WAREHOUSES_PRODUCT.findIndex(
      (wh_prod: any) =>
    ...
    if (INDEX_WAREHOUSE != -1) {
      this.toast.error(
        'VALIDACIÓN',
        'La existencia de ese producto con el almacen y la unidad ya existe'
      );
      return;
    }
    let data = {
    ...
    this.productWarehouseService
      .registerProductWarehouse(data)
      .subscribe((resp: any) => {
        ...
        this.toast.success(
          'EXITO',
          'La existencia de este producto se agrego correctamente'
        );
  ...
  editWarehouse(WAREHOUSES_PROD: any) {
    ...
    modalRef.componentInstance.WarehouseE.subscribe((wh_product: any) => {
      let INDEX = this.WAREHOUSES_PRODUCT.findIndex(
        (wh_prod: any) => wh_prod.id == WAREHOUSES_PROD.id
      );
      if (INDEX != -1) {
  ...
  addPriceMultiple() {
    if (!this.unit_price_multiple || !this.quantity_price_multiple) {
      this.toast.error(
        'VALIDACIÓN',
        'Necesitas seleccionar una unidad, aparte de colocar un precio'
      );
      return;
    }
    // unit_price_multiple
    ...
    let UNIT_SELECTED = this.UNITS.find(
      (unit: any) => unit.id == this.unit_price_multiple
    );
    let SUCURSALE_SELECTED = this.SUCURSALES.find(
      (sucurs: any) => sucurs.id == this.sucursale_price_multiple
    );
    let CLIENT_SEGMENT_SELECTED = this.CLIENT_SEGMENTS.find(
      (clisg: any) => clisg.id == this.client_segment_price_multiple
    );

    let INDEX_PRICE_MULTIPLE = this.WALLETS_PRODUCT.findIndex(
      (wh_prod: any) =>
        wh_prod.unit.id == this.unit_price_multiple &&
        wh_prod.sucursale_price_multiple == this.sucursale_price_multiple &&
        wh_prod.client_segment_price_multiple ==
    ...
    if (INDEX_PRICE_MULTIPLE != -1) {
      this.toast.error(
        'VALIDACIÓN',
        'El precio de ese producto con la sucursal y unidad ya existe'
      );
      return;
    }
    let data = {
    ...
    this.productWalletService
      .registerProductWallet(data)
      .subscribe((resp: any) => {
        console.log(resp);
        ...
        this.toast.success(
          'EXITO',
          'El precio de este producto se agrego correctamente'
        );
        this.isLoadingProcess();
        // {
        //   unit: UNIT_SELECTED,
  ...
  editProductWallet(WALLETS_PROD: any) {
    const modalRef = this.modalService.open(EditWalletPriceProductComponent, {
      centered: true,
      size: 'lg',
    });
    ...
    modalRef.componentInstance.WalletE.subscribe((wll_product: any) => {
      let INDEX = this.WALLETS_PRODUCT.findIndex(
        (wll_prod: any) => wll_prod.id == WALLETS_PROD.id
      );
      if (INDEX != -1) {
  ...
  removePriceMultiple(WALLETS_PROD: any) {
    const modalRef = this.modalService.open(DeleteWalletPriceProductComponent, {
      centered: true,
      size: 'md',
    });
    modalRef.componentInstance.WALLETS_PROD = WALLETS_PROD;
  ...
  addEspecif() {
    if (!this.key_v || !this.value_v) {
      this.toast.error(
        'VALIDACIÓN',
        'Necesitas digitar una propiedad y un valor'
      );
      return;
    }
  ...
  store() {
    if (
      ...
      this.toast.error(
        'VALIDACIÓN',
        'Necesitas llenar todos los campos obligatorios'
      );
      return;
    }
    ...
    this.productService
      .updateProduct(this.PRODUCT_ID, formData)
      .subscribe((resp: any) => {
        console.log(resp);
        if (resp.message == 200) {
