...
export class CreateProductComponent {
  ...
  //LISTA DE EXISTENCIAS DEL PRODUCTO EN ALMACENES
  WAREHOUSES_PRODUCT: any[] = [];
  //END SECTION WAREHOUSES

  // SECTION PRICE MULTIPLE
  ...
  removePriceMultiple(WALLETS_PROD: any) {
    ...
    // LA LISTA DONDE SE ENCUENTRA EL OBJECTO QUE QUIERO ELIMINAR
    //  OBTENER LA POSICIÓN DEL ELEMENTO A ELIMINAR
    let INDEX_WAREHOUSE = this.WALLETS_PRODUCT.findIndex(
      (wh_prod: any) =>
        wh_prod.unit.id == WALLETS_PROD.unit.id &&
        wh_prod.sucursale_price_multiple ==
          WALLETS_PROD.sucursale_price_multiple &&
        wh_prod.client_segment_price_multiple ==
          WALLETS_PROD.client_segment_price_multiple
    );
    //  LA ELIMINACIÓN DEL OBJECTO
    if (INDEX_WAREHOUSE != -1) {
  ...
  store() {
    ...
    if (this.WAREHOUSES_PRODUCT.length == 0) {
      this.toast.error(
        'VALIDACIÓN',
        'Necesitas ingresar al menos un registro de existencia de producto'
      );
      return;
    }
    if (this.WALLETS_PRODUCT.length == 0) {
      this.toast.error(
        'VALIDACIÓN',
        'Necesitas ingresar al menos un listado de precio al producto'
      );
      return;
    }
    ...
    formData.append(
      'WAREHOUSES_PRODUCT',
      JSON.stringify(this.WAREHOUSES_PRODUCT)
    );
