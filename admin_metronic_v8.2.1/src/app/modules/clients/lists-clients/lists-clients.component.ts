...
import { CreateActivityClientComponent } from '../create-activity-client/create-activity-client.component';
...
export class ListsClientsComponent {
  ...
  createClientActivity(){
    const modalRef = this.modalService.open(CreateActivityClientComponent,{
      centered: true,
      size: 'lg',
    })  
  }
  ...
  createClientCompany() {
    const modalRef = this.modalService.open(CreateClientsCompanyComponent, {
      centered: true,
      size: 'lg',
    });
  ...
  createClientPerson() {
    const modalRef = this.modalService.open(CreateClientsPersonComponent, {
      centered: true,
      size: 'lg',
    });
    modalRef.componentInstance.client_segments = this.client_segments;
  ...
  editClientCompany(CLIENT_SELECTED: any) {
    const modalRef = this.modalService.open(EditClientsCompanyComponent, {
      centered: true,
      size: 'lg',
    });
    ...
    modalRef.componentInstance.ClientsE.subscribe((client: any) => {
      let INDEX = this.CLIENTS.findIndex(
        (client_s: any) => client_s.id == CLIENT_SELECTED.id
      );
      if (INDEX != -1) {
  ...
  editClientPerson(CLIENT_SElECTED: any) {
    const modalRef = this.modalService.open(EditClientsPersonComponent, {
      centered: true,
      size: 'lg',
    });
    modalRef.componentInstance.client_selected = CLIENT_SElECTED;
    ...
    modalRef.componentInstance.ClientsE.subscribe(
      (clientE: any) => {
        let INDEX = this.CLIENTS.findIndex(
          (client: any) => client.id == CLIENT_SElECTED.id
        );
        if (INDEX != -1) {
  ...
  deleteClient(CLIENT_SELECTED: any) {
    const modalRef = this.modalService.open(DeleteClientsComponent, {
      centered: true,
      size: 'md',
    });
    modalRef.componentInstance.client_selected = CLIENT_SELECTED;

    modalRef.componentInstance.ClientsD.subscribe(
      (client_segment: any) => {
        let INDEX = this.CLIENTS.findIndex(
          (client_seg: any) => client_seg.id == CLIENT_SELECTED.id
        );
        if (INDEX != -1) {
